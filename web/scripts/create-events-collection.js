/* scripts/create-events-collection.js
 * Creates/updates the "events" collection with schema validation and indexes.
...
const eventsJsonSchema = {
  bsonType: "object",
  required: ["title", "slug", "type", "date", "location", "seats", "description", "status", "views"],
  properties: {
    title: {
      bsonType: "string",
      minLength: 3,
    ...
    type: {
      enum: ["Workshop", "Seminar", "Festival", "Tour", "Sports"],
    },
    date: {
      bsonType: "date",
    },
    ...
    image: {
      bsonType: "string",
      description: "Poster image URL or data URI",
    },
    status: {
  ...
  try {
    ...
    if (collections.length === 0) {
      await db.createCollection("events", {
        validator: { $jsonSchema: eventsJsonSchema },
      ...
      console.log('Updated validator on existing "events" collection');
    }
