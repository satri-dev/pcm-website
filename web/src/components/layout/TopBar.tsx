...
import { listTopBarLinks, getTopBarContact } from "@/repositories/topbar.repository";
    ...
    <div className="bg-pcm-blue-900 text-white/80">
      <div className="container flex items-center justify-between gap-2 py-2 px-3 sm:px-4 lg:px-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3 lg:gap-[1.1rem] text-[0.68rem] sm:text-xs lg:text-[0.8rem]">
          <a href={`tel:${contact?.phone || '061544761'}`} className="inline-flex items-center gap-1 sm:gap-[0.45rem] hover:text-pcm-green transition-colors">
            <Phone className="w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-[0.95rem] lg:h-[0.95rem] shrink-0" />
            <span className="hidden sm:inline">{contact?.phoneDisplay || '(061) 544761, 570124'}</span>
            <span className="sm:hidden">{contact?.phone || '(061) 544761'}</span>
          </a>
          <span className="w-px h-[1.1em] bg-white/28" />
          <a href={`mailto:${contact?.email || 'info@pcm.edu.np'}`} className="inline-flex items-center gap-1 sm:gap-[0.45rem] hover:text-pcm-green transition-colors">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-[1.1rem] ml-auto text-[0.68rem] sm:text-xs lg:text-[0.8rem]">
          ...
          {links.map((link) => (
            <div key={link.id}>
              {link.type === "simple" ? (
                <Link href={link.href} className="hidden md:inline hover:text-pcm-green transition-colors whitespace-nowrap">
                  {link.label}
                </Link>
              ) : (
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 sm:gap-2">
            <a href={contact?.facebookUrl || 'https://www.facebook.com/239069093193587'} target="_blank" rel="noopener" aria-label="Facebook" className="w-6 h-6 sm:w-[26px] sm:h-[26px] grid place-items-center border border-white/20 rounded-full hover:bg-pcm-green hover:border-pcm-green hover:text-pcm-navy transition-colors">
              <FaFacebook className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
            </a>
            <a href={contact?.instagramUrl || 'https://www.instagram.com'} target="_blank" rel="noopener" aria-label="Instagram" className="w-6 h-6 sm:w-[26px] sm:h-[26px] grid place-items-center border border-white/20 rounded-full hover:bg-pcm-green hover:border-pcm-green hover:text-pcm-navy transition-colors">
