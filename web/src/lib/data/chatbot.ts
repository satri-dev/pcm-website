...
export async function getPublicChatbotData(): Promise<PublicChatbotPayload> {
  ...
  try {
    ...
    // During build time or when DB is unavailable, return empty data
    console.warn("Failed to fetch chatbot data (this is expected during build):", error);
    return { entries: [], fallback: null };
  }
}
