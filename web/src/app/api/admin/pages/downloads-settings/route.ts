...
import {
  getDownloadsPageSettings,
  updateDownloadsPageSettings,
} from "@/repositories/downloads-settings.repository";
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const updated = await updateDownloadsPageSettings(parsed.data);
    revalidateTag(CACHE_TAGS.downloadsSettings, { expire: 0 });
    revalidatePath("/downloads");
    revalidatePath("/");
    return NextResponse.json(updated);
  } catch {
    return NextResponse.json(
      { error: "Failed to update downloads page settings" },
      { status: 500 }
    );
  }
}
