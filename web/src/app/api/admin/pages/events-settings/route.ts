...
import { getEventsPageSettings, updateEventsPageSettings } from "@/repositories/events-page-settings.repository";
...
const settingsSchema = z.object({
  ...
  sectionSubtitle: z.string().min(1).max(1500),
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const updated = await updateEventsPageSettings(parsed.data);
    revalidateTag(CACHE_TAGS.eventsSettings, { expire: 0 });
