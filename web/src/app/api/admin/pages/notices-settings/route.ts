...
import { getNoticesPageSettings, updateNoticesPageSettings } from "@/repositories/notices-page-settings.repository";
...
const settingsSchema = z.object({
  ...
  subscribeTitle: z.string().min(1).max(300),
  subscribeText: z.string().min(1).max(1000),
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const updated = await updateNoticesPageSettings(parsed.data);
    revalidateTag(CACHE_TAGS.noticesSettings, { expire: 0 });
