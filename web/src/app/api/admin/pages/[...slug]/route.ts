...
export async function PUT(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    await ensurePageContentsReady();
    const saved = await upsertPageContent(slugString, parsed.data as Record<string, unknown>);
    revalidateTag(CACHE_TAGS.pageContent(slugString), { expire: 0 });
...
export async function DELETE(_request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    const success = await deletePageContent(slugString);
    if (!success) {
      return NextResponse.json(
    ...
    console.error(`DELETE /api/admin/pages/${slugString}`, err);
    return errorResponse(err, "Failed to delete page content");
  }
}
