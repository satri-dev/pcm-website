...
import { getBlogPageSettings, updateBlogPageSettings } from "@/repositories/blog-page-settings.repository";
...
const settingsSchema = z.object({
  ...
  articlesEyebrow: z.string().min(1).max(200),
  articlesTitle: z.string().min(1).max(300),
  articlesSubtitle: z.string().min(1).max(1200),
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const updated = await updateBlogPageSettings(parsed.data);
    revalidateTag(CACHE_TAGS.blogSettings, { expire: 0 });
