...
export async function PATCH(
  ...
  if (action === "restore" || action === "permanent-delete") {
    ...
    if (action === "restore") {
      try {
        const restored = await restoreNotice(id);
        if (!restored) {
          return NextResponse.json({ error: "Not found" }, { status: 404 });
        }
    ...
    if (action === "permanent-delete") {
      try {
        const deleted = await hardDeleteNotice(id);
        if (!deleted) {
          return NextResponse.json({ error: "Not found" }, { status: 404 });
        }
        revalidateTag(CACHE_TAGS.noticesList, "max");
        ...
        return NextResponse.json(
          { error: "Failed to permanently delete notice" },
          { status: 500 }
        );
      }
    }
...
export async function DELETE(
  ...
  const { id } = await ctx.params;
  try {
    const deleted = await deleteNotice(id, guard.session.user.id);
    if (!deleted) {
      return NextResponse.json({ error: "Not found" }, { status: 404 });
    }
    revalidateTag(CACHE_TAGS.noticesList, "max");
