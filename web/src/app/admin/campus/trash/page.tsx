...
export default function CampusTrashPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/campus/trash");
  ...
  const getCollectionPath = (collection: string) => {
    if (collection === "facilities") return "campus/facilities";
    if (collection === "campus_map") return "campus/campus-map";
    return collection;
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">Campus Trash</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Trashed items from Facilities and Campus Map
          </p>
        </div>
        {items.length > 0 && (
          <button
