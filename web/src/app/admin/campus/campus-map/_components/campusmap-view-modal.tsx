...
interface CampusMapViewModalProps {
  open: boolean;
...
export default function CampusMapViewModal({
  open,
  onOpenChange,
  landmark,
}: CampusMapViewModalProps) {
  if (!landmark) return null;

  const statusBadge = landmark.status === "published" ? "green" : "gold";
      ...
      >
        ...
        <div className="modal__body">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-2">
            <span
              className={`badge badge--${categoryBadgeColor(landmark.category)}`}
            >
              {landmark.category}
            </span>
          ...
          <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 m-0 mb-4 text-sm">
            ...
            <dd className="m-0 text-[var(--admin-muted)]">
              {landmark.positionX}% horizontal, {landmark.positionY}% vertical
            </dd>
