...
interface ClubsManagerProps { initialData?: Club[]; }
...
export default function ClubsManager({ initialData }: ClubsManagerProps) {
  ...
  const handleSave = async (data: Club) => {
    setSaving(true);
    try {
      if (data.id && clubs.some((c) => c.id === data.id)) { await updateClub(data.id, data); } else { await createClub(data); }
    ...
    <main className="p-6">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">Manage, search and edit clubs and their members.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button type="button" className="admin-btn admin-btn--primary" onClick={handleAdd}><Plus size={16} /> Add Club</button>
        </div>
      </div>
      <div className="admin-panel">
        <div className="admin-panel__body p-0">
          {error ? <div className="p-6 text-[var(--admin-red)]">{error}</div> : <ClubsTable clubs={clubs} onAdd={handleAdd} onView={handleView} onEdit={handleEdit} onDelete={handleDelete} />}
        </div>
      </div>
