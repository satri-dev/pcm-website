...
export default function ClubsTable({ clubs, onAdd, onView, onEdit, onDelete }: ClubsTableProps) {
  ...
  const filtered = clubs.filter((item) => item.name.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-1/3 text-[var(--admin-muted)] hover:text-[var(--admin-brand)] font-bold text-[0.72rem] uppercase tracking-wider bg-[var(--admin-surface-2)] transition-colors cursor-pointer">CLUB</TableHead>
              <TableHead className="text-[var(--admin-muted)] hover:text-[var(--admin-brand)] font-bold text-[0.72rem] uppercase tracking-wider bg-[var(--admin-surface-2)] transition-colors cursor-pointer">ICON</TableHead>
              <TableHead className="text-[var(--admin-muted)] hover:text-[var(--admin-brand)] font-bold text-[0.72rem] uppercase tracking-wider bg-[var(--admin-surface-2)] transition-colors cursor-pointer">TAGLINE</TableHead>
              <TableHead className="text-[var(--admin-muted)] hover:text-[var(--admin-brand)] font-bold text-[0.72rem] uppercase tracking-wider bg-[var(--admin-surface-2)] transition-colors cursor-pointer">MEMBERS</TableHead>
          ...
          <TableBody>
            {paginated.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="text-center py-12"><div className="flex flex-col items-center"><Users size={40} className="opacity-30 mb-4" /><p>No records match your search.</p></div></TableCell></TableRow>
            ) : (
              paginated.map((item) => (
                  ...
                  <TableCell className="py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg overflow-hidden border border-[var(--admin-line)] flex-shrink-0 bg-[var(--admin-surface-2)]">
                        {item.image ? <img src={item.image} alt={item.name} className="w-full h-full object-cover" loading="lazy" /> : <span className="flex items-center justify-center h-full text-lg">{item.icon}</span>}
                      </div>
                      <div className="cell-main"><div className="font-semibold text-sm">{item.name}</div></div>
                    </div>
                  </TableCell>
                  <TableCell className="text-2xl py-3">{item.icon}</TableCell>
                  <TableCell className="text-sm py-3">{item.tagline}</TableCell>
                  <TableCell className="text-sm py-3">{item.members?.length ?? 0}</TableCell>
                  <TableCell className="py-3">
