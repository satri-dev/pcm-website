...
export default function AlumniManager({ initialData }: AlumniManagerProps) {
  ...
  const handleSave = async (data: Alumni) => {
    setSaving(true);
    try {
      if (data.id && alumni.some((a) => a.id === data.id)) {
        await updateAlumni(data.id, data);
      } else {
    ...
    <main className="p-6">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">Manage alumni records, batches, and career details.</p>
        </div>
      ...
      <div className="admin-panel">
        <div className="admin-panel__body p-0">
          {error ? (
            ...
            <AlumniTable alumni={alumni} onAdd={handleAdd} onView={handleView} onEdit={handleEdit} onDelete={handleDelete} />
          )}
        </div>
      </div>
