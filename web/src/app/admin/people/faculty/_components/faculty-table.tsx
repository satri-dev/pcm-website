...
import { Faculty, FACULTY_DEPARTMENTS } from "@/types/faculty";
...
export default function FacultyTable({ faculty, onAdd, onView, onEdit, onDelete }: FacultyTableProps) {
  ...
  const filtered = faculty.filter((item) => {
    const matchesSearch = item.name.toLowerCase().includes(searchQuery.toLowerCase()) || item.designation.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      <div className="flex items-center gap-3 p-5 border-b border-[var(--admin-line)] flex-wrap">
        ...
        <Select value={departmentFilter} onValueChange={(value) => setDepartmentFilter(value || "all")}>
          ...
          <SelectContent>
            ...
            {FACULTY_DEPARTMENTS.map((dept) => (<SelectItem key={dept} value={dept}>{dept}</SelectItem>))}
          </SelectContent>
        </Select>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-[var(--admin-muted)] hover:text-[var(--admin-brand)] font-bold text-[0.72rem] uppercase tracking-wider bg-[var(--admin-surface-2)] transition-colors cursor-pointer">DEPARTMENT</TableHead>
                ...
                <TableRow key={item.id} className="hover:bg-[#fafbfe] transition-colors">
                  <TableCell className="py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div><div className="font-semibold text-sm">{item.name}</div></div>
                    </div>
                  </TableCell>
                  <TableCell className="text-sm py-3"><span className="badge badge--blue">{item.department}</span></TableCell>
