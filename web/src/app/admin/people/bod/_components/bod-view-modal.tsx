...
interface BodViewModalProps { open: boolean; onOpenChange: (open: boolean) => void; bod: Bod | null; }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="news-modal w-[min(100%,640px)] sm:max-w-[640px] max-h-[90vh] overflow-y-auto flex flex-col gap-0 rounded-[16px] p-0 ring-0 outline-none">
        <div className="modal__head">
          <DialogTitle className="m-0 text-[1.05rem] font-normal flex items-center gap-2"><Eye size={18} /> Board Member</DialogTitle>
          <button type="button" className="admin-icon-btn" aria-label="Close" onClick={() => onOpenChange(false)}><X size={18} /></button>
        </div>
        <div className="modal__body">
          {row("Full name", bod.name)}
          {row("Role", bod.role)}
          {row("Order", String(bod.order ?? "—"))}
          {row("Photo", bod.photo ? <img src={bod.photo} alt="" style={{ maxWidth: 180, maxHeight: 110, borderRadius: 8, border: "1px solid #e2e7f0" }} /> : "—")}
        </div>
