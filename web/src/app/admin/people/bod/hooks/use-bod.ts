...
const API_BASE = "/api/admin/people/bod";
...
export function useBod({
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}?pageSize=${pageSize}`, { signal: controller.signal })
        ...
        if (!res.ok) {
          throw new Error(
            res.status === 401
              ...
              : `Failed to load BOD (HTTP ${res.status})`
          );
        }
        return res.json();
      })
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        setError(
          err instanceof Error ? err.message : "Failed to load BOD"
        );
        setLoading(false);
      });
    return () => {
  ...
  const updateBod = async (
    ...
    setBod((prev) => prev.map((item) => (item.id === id ? updated : item)));
    return updated;
  };
