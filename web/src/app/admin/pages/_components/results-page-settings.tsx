...
import {
  ...
  type ResultsPageSettings,
} from "@/types/results-page-settings";

const API_BASE = "/api/admin/pages/results-settings";
...
export default function ResultsPageSettings({
  initial,
}: {
  initial: ResultsPageSettings;
}) {
  const [form, setForm] = useState<ResultsPageSettings>({
    ...RESULTS_PAGE_SETTINGS_DEFAULTS,
    ...initial,
  });
  ...
  const set = <K extends keyof ResultsPageSettings>(
    key: K,
    value: ResultsPageSettings[K],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(
        "Saved. The public /results page now reflects these changes.",
      );
    } catch (err) {
    ...
    <main className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit every section of the public /results page — hero, section
            heading, results portal, and CTA band.
          </p>
        </div>
        <a
          className="admin-btn"
      ...
      <div className="admin-panel">
        ...
        <div className="admin-panel__body p-6 space-y-4">
          ...
          <label className="block">
            {fieldLabel("Portal Text")}
            <textarea
              rows={3}
              className={INPUT}
              value={form.portalText}
              onChange={(e) => set("portalText", e.target.value)}
            />
          </label>
          <label className="block">
            {fieldLabel("Portal URL")}
            <input
              className={INPUT}
              value={form.portalUrl}
              onChange={(e) => set("portalUrl", e.target.value)}
              placeholder="https://results.pu.edu.np/"
            />
          </label>
        </div>
      </div>
