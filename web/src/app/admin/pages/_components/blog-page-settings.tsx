...
const API_BASE = "/api/admin/pages/blog-settings";
...
export default function BlogPageSettings({
  ...
  initial: BlogPageSettings;
}) {
  const [form, setForm] = useState<BlogPageSettings>({
    ...BLOG_PAGE_SETTINGS_DEFAULTS,
    ...initial,
  });
  ...
  const set = <K extends keyof BlogPageSettings>(
    key: K,
    value: BlogPageSettings[K],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Saved. The public /blogs page now reflects these changes.");
    } catch (err) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit every section of the public /blogs page — hero, articles intro
            and CTA band. Blog posts are managed from Content · Media · Blogs.
          </p>
        </div>
        <a
          className="admin-btn"
        ...
        <div className="admin-panel__body p-6 space-y-4">
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => set("articlesEyebrow", e.target.value)}
            />
          </label>
          <label className="block">
            {fieldLabel("Title")}
            <input
              className={INPUT}
              value={form.articlesTitle}
              onChange={(e) => set("articlesTitle", e.target.value)}
            />
          </label>
          <label className="block">
            {fieldLabel("Subtitle")}
            <textarea
              rows={2}
              className={INPUT}
              value={form.articlesSubtitle}
              onChange={(e) => set("articlesSubtitle", e.target.value)}
            />
          </label>
        </div>
      </div>
            ...
            <div className="flex items-center gap-3">
              {form.ogImage && (
                ...
                <img
                  src={form.ogImage}
                  alt="OG"
                  className="h-16 w-28 object-cover rounded-lg border border-(--admin-line)"
                />
              )}
              <ImageUpload
                onUpload={(r) => set("ogImage", r.secure_url)}
              />
            </div>
            <span className="hint">Used for social sharing previews.</span>
          </label>
        </div>
      </div>
