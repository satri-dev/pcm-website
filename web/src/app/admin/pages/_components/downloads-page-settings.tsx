...
import { Save, Link2, Download } from "lucide-react";
import {
  DOWNLOADS_PAGE_SETTINGS_DEFAULTS,
  type DownloadsPageSettings,
} from "@/types/downloads-page-settings";

const API_BASE = "/api/admin/pages/downloads-settings";
...
const FIELDS: {
  key: keyof DownloadsPageSettings;
  label: string;
  ...
  { key: "searchPlaceholder", label: "Search placeholder", group: "section" },
...
const GROUP_TITLES: Record<"hero" | "section" | "cta", string> = {
  hero: "Page hero",
  section: "Downloads section heading",
  cta: "CTA band",
};

export default function DownloadsPageSettings({
  initial,
}: {
  initial: DownloadsPageSettings;
}) {
  const [form, setForm] = useState<DownloadsPageSettings>({
    ...DOWNLOADS_PAGE_SETTINGS_DEFAULTS,
    ...initial,
  });
  ...
  const set = (key: keyof DownloadsPageSettings, value: string) =>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Saved. The public /downloads page now reflects these changes.");
    } catch (err) {
  ...
  return (
    <main className="p-6">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit the heading, search hint and CTA text shown on the public
            /downloads page. The actual downloadable files are managed
            separately.
          </p>
        </div>
        <a className="admin-btn" href="/admin/media/downloads">
          <Download size={16} />
          Manage Downloads
        </a>
      </div>
              ...
              <p className="mt-0 mb-5 text-[0.85rem] text-[var(--admin-muted)]">
                {group === "cta"
                  ...
                  : group === "hero"
                    ...
                    : "Shown above the download file grid."}
              </p>
                    ...
                    {field.key.includes("Href") ? (
                      ...
                      <textarea
                        rows={
                          ...
                          field.key === "title" ||
                          field.key === "searchPlaceholder" ||
                          field.key === "ctaEyebrow" ||
                          field.key === "ctaTitle" ||
                          field.key.includes("Label")
                            ? 1
                            : 3
                        }
                        value={form[field.key]}
      ...
      <div className="mt-6 flex items-center gap-4">
        ...
        {message && (
          <span className="text-[0.9rem] text-[var(--admin-muted)]">
            {message}
          </span>
        )}
      </div>
