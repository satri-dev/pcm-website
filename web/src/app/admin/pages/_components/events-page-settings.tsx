...
const API_BASE = "/api/admin/pages/events-settings";
...
export default function EventsPageSettings({
  initial,
}: {
  initial: EventsPageSettings;
}) {
  const [form, setForm] = useState<EventsPageSettings>({
    ...EVENTS_PAGE_SETTINGS_DEFAULTS,
    ...initial,
  });
  ...
  const set = <K extends keyof EventsPageSettings>(
    key: K,
    value: EventsPageSettings[K],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(
        "Saved. The public /events page now reflects these changes.",
      );
    } catch (err) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit every section of the public /events page — hero, section
            heading and CTA band.
          </p>
        </div>
        <a
          className="admin-btn"
