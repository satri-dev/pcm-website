...
import {
  ...
  type MessagePageSettings,
} from "@/types/message-page-settings";

const API_BASE = "/api/admin/pages/message-settings";
...
export default function MessagePageSettings({
  initial,
}: {
  initial: MessagePageSettings;
}) {
  const [form, setForm] = useState<MessagePageSettings>({
    ...MESSAGE_PAGE_SETTINGS_DEFAULTS,
    ...initial,
  });
  ...
  const set = <K extends keyof MessagePageSettings>(
    key: K,
    value: MessagePageSettings[K]
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(
        "Saved. The public /about/message page now reflects these changes."
      );
    } catch (err) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className="m-0 text-2xl font-bold text-[var(--admin-ink)]">
            Leadership Message Page Content
          </h2>
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit the sections of the public /about/message page — hero, intro
            head and CTA band. The leadership messages themselves come from the
            Leadership Messages content manager.
          </p>
        </div>
        <a
          className="admin-btn"
        ...
        <div className="admin-panel__body p-6 space-y-4">
          ...
          <div className="rounded-xl border border-[var(--admin-line)] bg-black/[0.02] p-4">
            <p className="m-0 text-[0.9rem] text-[var(--admin-muted)]">
              The leadership cards shown below this section come from the
              Leadership Messages content manager.
            </p>
            <a className="admin-btn mt-3" href="/admin/people/leadership-message">
              Manage Leadership Messages →
            </a>
          </div>
        </div>
      </div>
