...
import { Save, Link2, MessageSquareText } from "lucide-react";
import {
  FAQ_PAGE_SETTINGS_DEFAULTS,
  type FaqPageSettings,
} from "@/types/faq-content";

const API_BASE = "/api/admin/pages/faq";
...
const FIELDS: {
  ...
  { key: "ctaSecondaryHref", label: "Secondary button link", group: "cta", hint: "e.g. /contact" },
];
...
export default function FaqPageSettings({
  ...
  const set = (key: keyof FaqPageSettings, value: string) =>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Saved. The public /faq page now reflects these changes.");
    } catch (err) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 mb-0 text-[0.9rem] text-[var(--admin-muted)]">
            Edit the heading, search hint and CTA text shown on the public /faq
            page. The actual questions and answers are managed separately.
          </p>
        </div>
        <a className="admin-btn" href="/admin/content/faqs">
