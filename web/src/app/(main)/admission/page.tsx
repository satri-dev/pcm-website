...
export async function generateMetadata(): Promise<Metadata> {
  const pageContentData = await getPageContent("admission");
  const content = (pageContentData?.content || {}) as Partial<AdmissionPageContent>;
  ...
  const ogImage = content.seo?.ogImage || DEFAULT_SEO.ogImage;
...
const DEFAULT_ADMISSION_CONTENT: AdmissionPageContent = {
  ...
  requiredDocuments: {
    ...
    scheduleBox: {
      ...
      examInfo: "Exam: <strong>Ashar 29, 2083 — 8:00 AM</strong>",
      deadlineInfo: "Form deadline: <strong>Ashar 26, 2083</strong>",
    },
  },
  applicationForm: {
...
export default async function AdmissionPage() {
  // Fetch admission page content with caching
  const pageContentData = await getPageContent("admission");

  // Extract content with defaults
  const existingContent = (pageContentData?.content || {}) as Partial<AdmissionPageContent>;

  // Merge with defaults using || for empty string handling
  const admissionData: AdmissionPageContent = {
    hero: {
  ...
  return <AdmissionClient content={admissionData} />;
}
