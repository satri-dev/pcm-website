...
import {
  programs,
  getProgramBySlug as getHardcodedProgram,
  getOtherPrograms,
} from "@/feature/Program/data/programs";
...
export async function generateStaticParams() {
  ...
  try {
    ...
    // Return at least the hardcoded programs
...
export default async function ProgramPage({
  ...
  const displayData = {
    ...
    image:
      dbProgram?.image ||
      hardcodedProgram?.image ||
      "/assets/img/program-bba.jpg",
    ...
    overviewTitle:
      ...
      hardcodedProgram?.overviewTitle ||
      "Program Overview",
    overviewBody:
      programPage.overview?.body || hardcodedProgram?.overviewBody || [],

    // Concentrations
    concentrations:
      programPage.concentrations || hardcodedProgram?.concentrations || [],
    ...
    admissionRequirements:
      programPage.admissionRequirements ||
      hardcodedProgram?.admissionRequirements ||
      [],
    ...
    quickFacts: {
      ...
      level:
        programPage.quickFacts?.level ||
        dbProgram?.level ||
        hardcodedProgram?.quickFacts?.level ||
        "Bachelor",
      duration:
        programPage.quickFacts?.duration ||
        dbProgram?.duration ||
        hardcodedProgram?.quickFacts?.duration ||
        "4 Years",
      semesters:
        programPage.quickFacts?.semesters ||
        dbProgram?.semesters ||
        hardcodedProgram?.quickFacts?.semesters ||
        8,
      creditHours:
        programPage.quickFacts?.creditHours ||
        dbProgram?.creditHours ||
        hardcodedProgram?.quickFacts?.creditHours ||
        0,
      eligibility:
        stripHtml(
          programPage.quickFacts?.eligibility ||
            dbProgram?.eligibility ||
            hardcodedProgram?.quickFacts?.eligibility,
        ) || "10+2",
      affiliation:
        programPage.quickFacts?.affiliation ||
        dbProgram?.affiliation ||
        hardcodedProgram?.quickFacts?.affiliation ||
        "Pokhara University",
      ...
      labels: {
        ...
        creditHours:
          programPage.quickFacts?.labels?.creditHours || "Credit hours",
        eligibility:
          programPage.quickFacts?.labels?.eligibility || "Eligibility",
        affiliation:
          programPage.quickFacts?.labels?.affiliation || "Affiliation",
      },
    },
    ...
    totalCredits:
      programPage.totalCredits ||
      hardcodedProgram?.totalCredits ||
      "0 Credit Hours",
    curriculumSection: {
      eyebrow: programPage.curriculumSection?.eyebrow || "Curriculum",
      title:
        programPage.curriculumSection?.title || "Program structure & syllabus",
      description:
        programPage.curriculumSection?.description ||
    ...
    // Coordinator
    coordinator: programPage.coordinator ||
      hardcodedProgram?.coordinator || {
    ...
    cta: {
      title:
        ...
        `Ready to apply for ${dbProgram?.code || slug.toUpperCase()}?`,
      body:
        programPage.cta?.body ||
        "Apply online in minutes, or reach out and we'll guide you through every step.",
      buttons: {
        primary: {
          text:
            programPage.cta?.buttons?.primary?.text ||
            `Apply for ${dbProgram?.code || slug.toUpperCase()}`,
        ...
        <div className="wrap-wide program-with-aside">
          ...
          <div>
            ...
            {displayData.growthSection?.title &&
              displayData.growthSection?.items?.length > 0 && (
                <>
                  <h3 className="program-section-h3">
                    {displayData.growthSection.title}
                  </h3>
                  <ul className="program-icon-list program-grid-2">
                    {displayData.growthSection.items.map(
                      (item: any, idx: number) => (
                        <li key={idx} className="program-icon-item">
                          <div className="program-icon-ic">
                            <StarIcon />
                          </div>
                          <div>
                            <h4
            ...
            {displayData.careers.length > 0 && (
              <>
                ...
                <p style={{ color: "var(--muted-c)" }}>
                  Graduates move into a wide range of professional roles,
                  including:
                </p>
                <div className="pill-row" style={{ marginTop: "0.6rem" }}>
                  {displayData.careers.map((career: string) => (
                    <span key={career} className="pill">
                      {career}
                    </span>
                  ))}
                </div>
            ...
            <div className="callout" style={{ marginTop: "2rem" }}>
              ...
              <p>
                {displayData.callout?.body ||
                  "Every semester includes non-credit courses that track current market demand for technology and skills — giving you an edge as you build toward your career path."}
              </p>
            </div>
            ...
            {displayData.admissionRequirements.length > 0 && (
              <>
                ...
                <ul
                  className="program-icon-list"
                  style={{ gap: "0.9rem", marginTop: "0.4rem" }}
                >
            ...
            <div className="program-aside-card">
              ...
              <div className="program-facts-list">
                {(
                  [
                    {
                      label: displayData.quickFacts.labels?.level || "Level",
                      value: displayData.quickFacts.level,
                    },
                    {
                      label:
                        displayData.quickFacts.labels?.duration || "Duration",
                      value: displayData.quickFacts.duration,
                    },
                    {
                      label:
                        displayData.quickFacts.labels?.semesters || "Semesters",
                      value: String(displayData.quickFacts.semesters),
                    },
                    {
                      label:
                        displayData.quickFacts.labels?.creditHours ||
                        "Credit hours",
                      value: String(displayData.quickFacts.creditHours),
                    },
                    {
                      label:
                        displayData.quickFacts.labels?.eligibility ||
                        "Eligibility",
                      value: displayData.quickFacts.eligibility,
                    },
                    {
                      label:
                        displayData.quickFacts.labels?.affiliation ||
                        "Affiliation",
                      value: displayData.quickFacts.affiliation,
                    },
                  ] as const
                ).map((fact, i, arr) => (
                  <div
                    key={fact.label}
                    className={`program-fact-row${i === arr.length - 1 ? " last" : ""}`}
                  >
              ...
              >
                {displayData.cta?.buttons?.primary?.text ||
                  `Apply for ${displayData.badge}`}{" "}
                <ArrowRight />
              </a>
              <a
          ...
          <div className="wrap-wide">
            <div className="section-head">
              <span className="eyebrow">
                {displayData.curriculumSection.eyebrow}
              </span>
            ...
            <div style={{ marginTop: "2.2rem" }}>
              <CurriculumTabs
                semesters={displayData.curriculum}
            ...
            <article className="leader-card" style={{ marginTop: "2.5rem" }}>
              <div className="leader-card__media">
                <Image
                  ...
                  loading="lazy"
                />
                <span className="leader-card__chip">
                  {displayData.coordinator.initials}
                </span>
              </div>
              ...
              <div>
                ...
                <h2>
                  {displayData.cta?.title ||
                    `Ready to apply for ${displayData.badge}?`}
                </h2>
                <p>
                  {displayData.cta?.body ||
                    "Apply online in minutes, or reach out and we'll guide you through every step."}
                </p>
              </div>
