...
import type { ResultsPageSettings } from "@/types/results-page-settings";
import type { Result as DbResult } from "@/types/results";
...
interface Props {
  settings: ResultsPageSettings;
  resultItems: DbResult[];
}

export default function ResultsClient({ settings, resultItems }: Props) {
  ...
  const popular = resultItems.slice(0, 4).map(r => ({
  ...
  const filtered = resultItems.filter((r) => {
    const matchesProgram = program === "all" || r.program?.toLowerCase() === program;
      ...
      <main id="main">
        ...
        <section className="section">
          <div className="wrap-wide" style={{ maxWidth: 900 }}>
            <div className="section-head reveal"><span className="eyebrow">{settings.sectionEyebrow}</span><h2 className="section-title">{settings.sectionTitle}</h2></div>
            ...
            <div className="tabs reveal">
              ...
              <div className={`tabs__panel${tab === "latest" ? " active" : ""}`}>
                  <div className="page-tools">
                    ...
                    <span className="page-tools__count">{filtered.length} of {resultItems.length} shown</span>
                  </div>
                  ...
                  <div className="result-list" style={{ marginTop: "1.8rem" }}>
                    {visible.map((r) => (
                      <article key={r.title} className="card result-item reveal is-inview">
                        ...
                        <div>
                          <span className="result-item__meta">{r.program} · Published {r.date}</span>
                          <h3><a href={r.fileUrl || "#"}>{r.title}</a></h3>
                        </div>
            ...
            <div className="card" style={{ marginTop: "2rem", padding: "2rem", textAlign: "center" }}>
              <h3>{settings.portalTitle}</h3>
              <p style={{ color: "var(--muted)", margin: ".5rem 0 1.2rem" }}>{settings.portalText}</p>
              <a className="btn btn-primary" href={settings.portalUrl} target="_blank" rel="noopener">Visit results.pu.edu.np {ArrowRight}</a>
            </div>
          </div>
        </section>
